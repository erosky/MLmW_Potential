...
# desired output:
# z, Pxx(z), Pyy(z), Pzz(z), Pn(z), Pt(z), N, P_tot
...
BEGIN { Z = 100 ; thickness = 6 ; V = 15000 ; start = 0 ; t = 0 ; Pxx = 0 ; Pyy = 0 ; Pzz = 0 } 
{
	...
	if (a==1 && ($5>Z-thickness/2 || $5<Z+thickness/2-100) && ($5<Z+thickness/2 || $5>100+Z-thickness/2)) { v_Pxx += $14 ; v_Pyy += $15 ; v_Pzz += $16 ; v_n++ }
}
END { print Z "\t\t" (-Pxx/t) "\t\t" (-Pyy/t) "\t\t" (-Pzz/t) "\t\t" (-Pzz/t) "\t\t" (-0.5*(Pxx/t + Pyy/t)) "\t\t" (n/V)/t "\t\t" (-(Pxx/t+Pyy/t+Pzz/t)/(3)) }
